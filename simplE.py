...
class SimplEModule(BaseModule):
    def __init__(self, n_ent, n_rel, config):
        ...
        self.rel_inv_embed = nn.Embedding(n_rel, config.dim)
        self.ent_head_embed = nn.Embedding(n_ent, config.dim)
        self.ent_tail_embed = nn.Embedding(n_ent, config.dim)
        for param in self.parameters():
            param.data.div_((config.dim / sigma ** 2) ** (1 / 6))

    def forward(self,src,rel,dst):
        return (t.sum(self.ent_head_embed(src)* self.rel_re_embed(rel)*self.ent_tail_embed(dst),dim=-1)+t.sum(self.ent_head_embed(dst)*self.rel_inv_embed(rel)*self.ent_tail_embed(src),dim=-1))/2.0
...
class SimplE(BaseModel):
    def __init__(self, n_ent, n_rel, config):
        ...
        self.mdl = SimplEModule(n_ent, n_rel, config)
